...
float_bits float_negate_true(float_bits f) {
	float fn = (- *((float *) &f));
	if (isnan(fn)) return f;
	return *((float_bits *) &fn);
}
...
void assert(unsigned x) {
	if (float_negate(x) != float_negate_true(x)) 
	printf("Test failed: \nTest case: 0x%x, MY: 0x%x, ANS: 0x%x\n", 
		x, float_negate(x), float_negate_true(x));
}
